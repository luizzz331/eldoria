...
package 
{
    ...
    public class LPFFrameItemPreview extends LPFFrame 
    {
		...
		private var sLinkShield:String = "";
		private var sLinkNecklace:String = "";
                ...
                if (curItem != _arg_1)
                {
                    ...
                    switch (_arg_1.sES)
                    {
                        ...
                        case "ba":
                            ...
                            break;
						case "am":
							loadNecklace(_arg_1.sFile, _arg_1.sLink);
						break;
						case "sh":
							loadShield(_arg_1.sFile, _arg_1.sLink);
						break;
                        case "pe":
		...
		private function loadShield(file:*, linkage:*):void
        {
            clearPreview();
            sLinkShield = linkage;
            var loader:* = new Loader();
            loader.load(new URLRequest(Game.serverFilePath + file), pLoaderC);
            loader.contentLoaderInfo.addEventListener(Event.COMPLETE, onShieldLoadComplete, false, 0, true);
            addToLoaderStack(loader);
        }
		
		private function onShieldLoadComplete(event:Event):void
        {
            removeFromLoaderStack(event.target);
            var _local_2:Class = (pLoaderD.getDefinition(sLinkShield) as Class);
		...
		private function loadNecklace(file:*, linkage:*):void
		{
            clearPreview();
            sLinkNecklace = linkage;
            var _local3:* = new Loader();
            _local3.load(new URLRequest(Game.serverFilePath + file), pLoaderC);
            _local3.contentLoaderInfo.addEventListener(Event.COMPLETE, onLoadNecklaceComplete, false, 0, true);
            addToLoaderStack(_local3);
        }
		
		private function onLoadNecklaceComplete(event:Event):void
		{
            removeFromLoaderStack(event.target);
            var _local_2:Class = (pLoaderD.getDefinition(sLinkNecklace) as Class);
            var _local_3:* = new (_local_2)();
            _local_3.scaleX = (_local_3.scaleY = 2);
            mcPreview.addChild(_local_3);
            addGlow(_local_3);
        ...
        private function clearLoaderStack():void
        {
            ...
            while (loaderStack.length > 0)
            {
                ...
                try
                {
                    ...
                    _local_1.removeEventListener(Event.COMPLETE, onLoadNecklaceComplete);
                    _local_1.removeEventListener(Event.COMPLETE, onShieldLoadComplete);
                    _local_1.close();
                }
                catch(e:Error)
                {
